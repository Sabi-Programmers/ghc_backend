...
function splitName(fullName) {
  ...
  // Initialize an object to hold the name components
  const nameObj = {
    firstName: "",
    middleName: "",
    lastName: "",
  ...
  // Check the length of the array and assign names accordingly
  if (nameParts.length === 2) {
    // If there are two parts, assign them as first name and last name
    nameObj.firstName = nameParts[0];
    nameObj.lastName = nameParts[1];
  } else if (nameParts.length === 3) {
    // If there are three parts, assign them as first name, middle name, and last name
    nameObj.firstName = nameParts[0];
    nameObj.middleName = nameParts[1];
    nameObj.lastName = nameParts[2];
  } else {
    // If there's a different number of parts, return the full name as first name
    nameObj.firstName = fullName;
  }
