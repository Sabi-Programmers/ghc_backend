window.onload = function () {
    ...
    if (newsEditor && newsForm) {
        ...
        const newsFormSubmitBtn = document.getElementById(
    ...
    if (currentRoute === '/packages') {
        const selectPackageForm = document.getElementById(
            'select-package-form',
        );
    ...
    if (currentRoute === '/packages/complete-order') {
        ...
        const packageOrderError = document.querySelector(
            '#package-order-error',
        );
        ...
        const renderPackageOrderItems = () => {
            ...
            const removePackageBtn = document.querySelectorAll(
                '.remove-package-btn',
            );
            ...
            packageQty.forEach((pkgInput, i) => {
                pkgInput.addEventListener('change', () => {
                    ...
                    packages[i].subTotal =
                        packages[i].price * packages[i].quantity;
                    renderPackageOrderItems();
                });
            });
    ...
    const displayNews = async () => {
        ...
        const modalBody = document.querySelector(
            '#news-modal .modal-content-body',
        );
        const modalContentTitle = document.querySelector(
            '#news-modal .modal-content-title',
        );
        const modalNewsImg = document.querySelector(
            '#news-modal .modal-news-img',
        );
        ...
        const filterReadNews = (news) => {
            ...
            return news.filter(
                (newsItem) => !readNewsIds.includes(newsItem.id),
            );
        };
    ...
    />`;
        ...
        modalCloseBtn.addEventListener('click', () => {
            if (modalReadNews.checked) {
                let readNewsIds =
                    JSON.parse(localStorage.getItem('readNewsIds')) || [];
                readNewsIds = readNewsIds.concat(
                    news.map((newsItem) => newsItem.id),
                );
                localStorage.setItem(
                    'readNewsIds',
                    JSON.stringify(readNewsIds),
                );
            }
            modal.hide();
        });
